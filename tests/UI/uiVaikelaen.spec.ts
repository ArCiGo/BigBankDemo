...
test.describe('Väikelaen calculator modal', () => {
    test('The user can update the parameters `amount` and `period` in the URL, and the calculator modal should load with these values', async ({ page }) => {
        vaikelaenPage = new VaikelaenPage(page);

        await test.step('STEP 1: Navigate to the page with custom `amount` and `period` URL parameters', async () => {
            const { amount, period } = vaikelaenData.urlParams;
            await page.goto(`/?amount=${amount}&period=${period}&productName=SMALL_LOAN&loanPurpose=DAILY_SETTLEMENTS&lang=ee`);
        });

        await test.step('STEP 2: Verify the modal loaded with the values set in the URL', async () => {
            await expect.poll(async () => {
                return await vaikelaenPage.vaikelaenCalculatorModalValidation();
            }).toEqual(expect.objectContaining({
                laenusumma: vaikelaenData.urlParams.modalLaenusumma,
                periood: vaikelaenData.urlParams.modalPeriood,
            }));
        });
    });

    // Tests below share the same default URL
    test.describe('With default URL', () => {
        test.beforeEach(async ({ page }) => {
            await test.step('STEP 0: Open the Väikelaen page', async () => {
                await page.goto('/?amount=5000&period=60&productName=SMALL_LOAN&loanPurpose=DAILY_SETTLEMENTS&lang=ee');
            });
        });
