...
In my judgment, the following scenario groups are necessary to verify that the calculator modal works as expected.-
...
I applied boundary value analysis and equivalence partitioning for both UI and API testing, since the inputs (**amount** and **maturity**) have clearly defined numeric ranges. For the UI, I complemented this with exploratory testing to uncover unexpected behaviors. I prioritized functional testing because the calculator's core value is the accuracy of its outputs given user inputs.
...
* Updating, saving and validating values in the calculator modal.
...
For the UI, I automated three scenarios. The first one is the save and edit flow: It covers **TC-UI-009** and **TC-UI-003**, but also validates the default state implicitly: After navigating to the page, the test clicks **Jätka** with the prefilled values and immediately checks that the header shows *5000 €*, which confirms the modal opened with the correct defaults without needing a separate test for it. 

The second scenario covers clamping behavior (**TC-UI-004**, **TC-UI-005**, **TC-UI-007**, **TC-UI-008**): I handled all four boundary cases in a single test because the logic is the same for both fields: Type an out of range value, blur the input, and verify the field snaps back.

I left **TC-UI-002** (URL parameters), **TC-UI-006** (no decimals in Periood), and **TC-UI-011** (invalid strings) as «manual». They are all single-step verifications that are faster to check visually than to maintain a test for. The slider test (**TC-UI-010**) exists in the code as a commented-out demo step.
...
* **A CI/CD pipeline** that is simple enough to read, extend, and debug without needing to be a DevOps expert.
* **Documentation** that explains how to set up, run, and extend the project — including what each folder is for.

The goal is that someone new to the project can clone the repository, read the README, and be running tests within a few minutes without asking anyone for help.
...
* I noticed that some HTML elements have their own `data-testid` attributes (and that is good!), but their values are not unique and meaningful. For example: For the **Laenusumma**, I can see the `data-testid` value is `bb-input__input`, but for the `id` is `header-calculator-amount`. There are more than 8 HTML elements in the page referencing to the same `data-testid` value. I would suggest to use more specific and unique `data-testid` values for each element.
  * For further reading, this resource explains well why `data-testid` attributes are better for automation testing: [Why Should You Use data-testid Attributes?](https://bugbug.io/blog/software-testing/data-testid-attributes/).
